...
exports.BlockFriend1= (req, res) => {
console.log("start ......BlockFriend1");
var inputs=req.body;
    ...
    Note.find({
       ...
       if(note.length==2)
       {
            ...
            Friend.find({
                ...
                {
                    var flag=0;
                    requested_lst=request[0].requested_list;
                    accepted_lst=request[0].accepted_list;
                    requested_byme_lst=request[0].requested_by_me;
                    blocked_lst=request[0].blocked_list;
                ...
                else
                {
                    ...
                    .then(data=>{
                        insert_data();
                        //res.status(200).send({result:"success",message:"Blocked friend successfully"}); 
                    }).catch(err => {
                        res.status(500).send({result:"failed",message:"There was an exception",errorMessage: err.message});
                    });
                }
            ...
            function insert_data()
            {
                ...
                Friend.find({
                    ...
                    if(request.length!=0)
                    {
                        ...
                        requested_byme_lst=request[0].requested_by_me;
                        blocked_lst=request[0].blocked_list;
                        blocked_otheruser=request[0].blocked_by_others;
                        ...
                        if(flag==1)
                        {
                            ...
                            const updatedevice=Friend.updateOne( 
                                {user_id: toid}, 
                                {
                                    ...
                                    requested_list:requested_lst,
                                    requested_by_me:requested_byme_lst,
                                    blocked_by_others:blocked_otheruser
                                },function(err,updatedevice) {
                                if(err)
                                { 
                                    return res.status(500).send({result:"failed",message:"There was a problem adding the information to the database."});
                                ...
                                {
                                    return res.status(200).send({result:"success",message:"Blocked friend successfully"});   
                                }                          
                                }).catch(err => {
                                    res.status(500).send({result:"failed",message:"There was an exception",errorMessage: err.message});
                                });
