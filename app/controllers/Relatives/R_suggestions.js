...
exports.relativeSuggestions = (req, res) => {
    console.log("start.....relativeSuggestions");
       ...
       {   
           ...
           function findids()
           {
              Relative.find({
                ...
                {
                    var temp_ids=[];
                    var acc_list=request[0].r_accepted_list;
                    for(var i=0;i<acc_list.length;i++){
                        temp_ids.push(acc_list[i].id);
                    }
                    //to find relatives of relatives *temp_ids are relatives of user_id
                    Relative.find({
                        "user_id":{"$in": temp_ids},
                    }).then(request11=>{
                    console.log("request11.length",request11.length)
                        ...
                        if(request11.length>1)
                        {
                            ...
                            {
                                console.log("j----",j);
                                var acc_list=request11[j].r_accepted_list;
                        ...
                        if(request11.length==1)
                        {
                            ...
                            var acc_list=request11[0].r_accepted_list;
    ...
    function show_list()
    {
        ...
        Note.find({
            ...
            { 
                ...
                return res.status(200).send({result:"success",message:"No relatives list found",data:[]}); 
            } 
            else
            {
