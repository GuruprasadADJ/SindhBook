    ...
    {
        return res.status(200).send({result:"failed",message: "Content can not be empty"});
    }
    ...
    if(note.length == 0){
    res.status(200).send({result:"failed",message: 'login failed, User not found'});
    }else{
    ...
    res.status(200).send({result:"failed",message : "login failed, verify mobile number",otp_verified_status:otp_status});
    }
    }
    ...
    res.status(500).send({
        ...
        if(note.length == 0){
        res.status(200).send({result:"failed",message: 'login failed, This account is not registered'});
        }else{
        var pupdate_status=note[0].profile_update_status;
        if(pupdate_status==1 ){
        res.status(200).send({result:"success",message : "login successful"});
